...
def exec_commands(api_instance):
    ...
    resp = None
    try:
        resp = api_instance.read_namespaced_job(name=name,
                                                namespace='default')
    ...
    if not resp:
        ...
        api_instance.create_namespaced_job(namespace= 'api-slicing',body=job_body)
        print("Done. The test is now running :)")
        time.sleep(380)
        print("The test is now over!")
        api_instance.delete_namespaced_job(name=name, namespace = 'api-slicing')
