...
def exec_commands(api_batch, api_core):
    ...
    resp = None
    try:
        resp = api_batch.read_namespaced_job(name=name,
                                                namespace='default')
    ...
    if not resp:
        ...
        for i in pods_list_items:
            api_core.delete_namespaced_pod(name= i.metadata.name, namespace = 'api-slicing')
        print("All pods are being deleted")
def main():
    auth_token = ("<get token with: kubectl describe secret -n api-slicing api-slicing-secret>")
